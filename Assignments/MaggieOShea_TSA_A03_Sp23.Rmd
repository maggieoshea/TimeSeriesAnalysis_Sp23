...
deSeas_biomass <- energy_clean$`Total Biomass Energy Production` - biomass_seas_comp
deSeas_RE <- energy_clean$`Total Renewable Energy Production` - RE_seas_comp
deSeas_hydro <- energy_clean$`Hydroelectric Power Consumption`- hydro_seas_comp

#Plot 
ggplot(energy_clean, aes(x=Month, y=`Total Biomass Energy Production`))+
  geom_line(color="blue")+
  geom_line(aes(y=deSeas_biomass), color="orange")+
  labs(color = "legend")

ggplot(energy_clean, aes(x=Month, y=`Total Renewable Energy Production`))+
  geom_line(color="blue")+
  geom_line(aes(y=deSeas_RE), color="orange")
...
```{r}
#Biomass 
acf(deSeas_biomass, lag.max=40)
#acf(energy_clean$`Total Biomass Energy Production`, lag.max = 40)
pacf(deSeas_biomass, lag.max=40)
#pacf(energy_clean$`Total Biomass Energy Production`, lag.max = 40)

#Renewable Energy
acf(deSeas_RE, lag.max=40)
#acf(energy_clean$`Total Renewable Energy Production`, lag.max = 40)
pacf(deSeas_RE, lag.max=40)
#pacf(energy_clean$`Total Renewable Energy Production`, lag.max = 40)
