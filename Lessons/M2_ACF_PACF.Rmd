...
#Pasting column 1 indicated by [,1] and column 2 [,2]
my_date <- my(my_date)  #function my from package lubridate 
#it says: my because it means month year, so if you want to do year, month then it would be ym
head(my_date)
...
#b means written month so from 01 to Jan

## Test
my_date2 <- paste(raw_inflow_data[,2],raw_inflow_data[,1],sep="-")
my_date2 <- ym(my_date2)
head(my_date2)
head(my_date)
```
...
#histograms can help with probability function
#Distribution: log-normal distribution, no negative numbers and left-skewed
...
#[,2] says you start at the 2nd column because you don't want to convert the first column because it's the date column --> then you do 2:2+nhydro-1 which is column 2 up to column 2+3-1 which is column 4
...
#Each bar represents auto-correlation
#ACF will always range between 1 and -1 
#This plot shows a seasonal component 
#This plot shows up to lag 30 which is a lot 
#Blue dotted lines are significant levels, so anything within those lines is not significant enough 
