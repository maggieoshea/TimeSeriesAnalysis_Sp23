...
```{r}
SSES_seas <- ses(ts_price,model="ZZZ",h=n_for,holdout=FALSE,silent="none")
plot(SSES_seas)
...
```{r}
SS_seas_na <- StructTS(ts_price,
                    type="BSM",fixed=c(NA, NA, NA,NA)) 
SS_seas_na$coef

checkresiduals(SS_seas_na)
#You can see in the ACF that there is seasonal correlations. Problem with ACF is that the errors are correlated, where they seem to have strong correlation happening so you can go back and play with values of variance to try to fix this 

SS_seas <- StructTS(ts_price,
                    type="BSM",fixed=c(0,0.001,0.3,NA))   #this function has convergence issues
#In this case you're giving it a starting point, so function doesn't have to tell or figure out the variance
#The first variance is the level of the series
#The second is the trend variance
#The third is seasonal variance
#The fourth is variance associated with observations
...
# Model 4:  SSES
SSES_scores <- accuracy(SSES_seas$mean,last_obs)
## Changed this to mean away from forecast because forecast wasn't working 
...
autoplot(ts_electricity_price[,"All.sectors"]) +
  ...
  autolayer(SSES_seas$mean, series="SSES") +
