...
More granular data has more complex seasonality going on
  - Not just winter-summer but seasonality possible within a week (ex: weeknight vs. weekday) 
  - Hourly will have day-night seasonality as well as monthly seasonality 
      - Hourly data has daily pattern, weekly pattern, and monthly/annual pattern
  - Many models can only happen one type of seasonality 
      - To apply ARIMA model to a complex time series, then you have to do something with the seasonality
      - State Space can handle more seasonality but will need more equations 
  - TS function, if you have more than one seasonality you have to choose what frequency to specify (365 for daily, or 12 for monthly seasonality)
      - if  you want to do both then you have to use msts which stands for multiple seasonality time series 
...
unlink(temp)
##unz = unzips file from online, the name of the file is listed because the zip could have many files in it 

#Fixing date columns and selecting active power only column
act_power_all <- household_all %>% 
  mutate( Global_active_power = as.numeric(Global_active_power)) %>%  
  mutate( Date = dmy(Date)) %>% 
  mutate( Time = hms(Time)) %>% #hms =  hour minute second which makes it essentially into a date object that can be broken down as seen below
...
ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            ...
                            start=c(2006,12,16))
#seasonal periods for the hourly data are the values of hourly (24), weekly (168 which is 24 hours*7 days a week), yearly (8766 which is 24*365.25)
#If you don't specify anything, the default frequency will choose the max 

ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, 
                           seasonal.periods =c(7,365.25),
                           start=c(2006,12,16))
#Because this is day not hourly data, then you only do the 7 for weekly patterns and 365.25 which will cover the annual pattern
...
ts_act_power_daily %>% mstl() %>%
  autoplot()
#The lighter/darker one is the weekly pattern, it's darker because there are more observations in that area in between the lighter ones (zoom to see)  

ts_act_power_hourly %>% mstl() %>%
  autoplot()
#There are more plots here because after the trend its the hourly seasonality, the weekly seasonality, then the yearly seasonality 
...
##For training you can make one that is the same as your original msts with just one year less 
n_for = 365
...
#For testing you can take just the last 365 observations 
...
ARIMA_Four_fit <- auto.arima(ts_act_power_daily_train, 
                             seasonal=FALSE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, #xreg adds exogenous variables, but in this case it's a seasonal component, and don't know how many terms there will be etc. so fourier will search to find the best one but you have to give it some options like 2 (seasonal weekday and weekend) and 12 (monthly)
                                          K=c(2,12))
                             )
#xreg --> the way we can use exogenous variables and incorporate our 2 seasonalities which is why we say seasonal = FALSE,even though we do have seasonality because it's incorporated as a regressor 
    #Call the fourier function, just the terms necessary to model the seasonality, so for daily its 2 coefficients because its weekday or weekend, and then for each year its 12 because it's monthly (or you could do 4 because there are 4 seasons in the year) 
...
#but here you need a forecast too not just a fit even for the regressors which is why you have h=365
...
```{r}
plot(rnorm(4, mean=3500, sd=5000))
```
