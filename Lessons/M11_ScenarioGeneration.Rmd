...
#Need 3 variables
nscen=1000
#This is scenarios = 1000 scenarios 

#Generate 1000 normal random variates for each of 3 variables 
...
R[2,3]=0.85
R[3,2]=0.85
#This should be the correlation among 3 variables according to historical data 

# Get Cholesky decomposition, chol() will give upper triangular matrix with the values that we input (R matrix) 
U=chol(R)

# Transpose U to get lower triangular matrix just 
L=t(U) 
#If you multiple L * U you should get the same correlation matrix that you had before (R)
L%*%U
...
```
Drawing from 2 different variables and you have the distribution, you need the joined distribution in order to get the correlations be what you want - so it's easier to use the Cholesky decomposition (method above) which involves matrices 
