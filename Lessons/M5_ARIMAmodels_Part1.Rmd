...
ACF_Plot <- Acf(electricity_price_processed$Residential, lag = 40, 
 ...
 # this looks like an AR because there is slow decay, showing the memory 
PACF_Plot <- Pacf(electricity_price_processed$Residential, lag = 40, 
                  main = "PACF Residential Electricity Price")
#Strong AR model which you can see because the first lag has an almost 1 correlation 
```
...
#This takes your original series and subtracts your seasonal component 
    #It only works on decompose object 
...
#Reject so it is not stationary, and it has a unit root --> unit root means we need to difference (in addition to seasonal adjusting) 
...
##No more unit root, you can see because the ACF no longer has the first lag so highly correlated
...
#drift says even after differencing the mean may not be exactly zero so take that into account
...
## Best model is PACF and ACF has all correlations within the blue lines 
...
## What happens if you don't difference?
